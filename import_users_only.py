...
def parse_mysql_insert(statement):
    ...
    # Skip tables we don't have mapping for
    if not pg_table or mysql_table.startswith('legacy_') or mysql_table.startswith('occupational_'):
        return None
...
def main():
    ...
    # Order of tables to import (respecting foreign keys)
    import_order = [
    ...
    # Process each INSERT statement in order
    for mysql_table in import_order:
        ...
        for stmt in statements:
            ...
            if f'INSERT INTO `{mysql_table}`' not in stmt:
                continue
            ...
            # Insert each row individually to avoid issues
            for row in rows:
                try:
    ...
    for table in tables_with_id:
        try:
            ...
            # Just skip if sequence or table doesn't exist
            conn.rollback()
            pass
