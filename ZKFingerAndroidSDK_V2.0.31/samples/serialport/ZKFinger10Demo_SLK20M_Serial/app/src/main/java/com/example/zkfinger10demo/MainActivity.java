...
import static com.zkteco.android.biometric.module.fingerprint.FingerprintCaptureListener.MODE_CAPTURE_TEMPLATE;

public class MainActivity extends AppCompatActivity {
    private static final int ZKTECO_VID =   0x1b55;
    private static final int LIVE20M_PID =   0x0121;
    ...
    private String serialName = "/dev/ttyS0";
    ...
    private FingerprintCaptureListener fingerprintCaptureListener = new FingerprintCaptureListener() {
        @Override
        public void captureOK(int mode, byte[] rawImage, int[] attributes, byte[] fpTemplate) {
            if (MODE_CAPTURE_TEMPLATEANDIMAGE == mode)
            {
                final int width = attributes[0];
                final int height = attributes[1];
                final Bitmap bitmap = ToolUtils.renderCroppedGreyScaleBitmap(rawImage, width, height);
            ...
            final int templateLength = fingerprintSensor.getLastTempLen();
            if (bRegister)
            {
    ...
    private void createFingerprintSensor()
    {
        ...
        //set vid
        deviceParams.put(ParameterHelper.PARAM_SERIAL_BAUDRATE, 115200);
        //set pid
        deviceParams.put(ParameterHelper.PARAM_SERIAL_SERIALNAME, serialName);
        fingerprintSensor = FingerprintFactory.createFingerprintSensor(getApplicationContext(), TransportType.SERIALPORT, deviceParams);
    }
    ...
    private void openDevice()
    {
        ...
        try {
            ...
            {
                textView.setText("init zkfingerservice failed!");
                return;
            }
            fingerprintSensor.setFingerprintCaptureMode(deviceIndex, MODE_CAPTURE_TEMPLATE);
            ...
            {
                // device parameter
                LogHelper.d("sdk version" + fingerprintSensor.version());
            ...
            try {
                fingerprintSensor.rebootDeviceEx(deviceIndex);
            } catch (FingerprintSensorException ex) {
                ex.printStackTrace();
            }
    ...
    public void onBnStart(View view)
    {
        ...
        serialName = editSerial.getText().toString();
        if (null == serialName || serialName.isEmpty())
        {
            textView.setText("Please input serial name!");
            return;
        }
        openDevice();
    }
